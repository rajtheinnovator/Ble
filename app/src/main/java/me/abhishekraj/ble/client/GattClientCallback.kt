...
class GattClientCallback(private val mClientActionListener: GattClientActionListener) : BluetoothGattCallback() {
    ...
    override fun onDescriptorWrite(gatt: BluetoothGatt, descriptor: BluetoothGattDescriptor, status: Int) {
        if (status == BluetoothGatt.GATT_SUCCESS) {
            mClientActionListener.log("Descriptor written successfully: " + descriptor.uuid.toString())
            mClientActionListener.initializeTime()
        } else {
            mClientActionListener.logError("Descriptor write unsuccessful: " + descriptor.uuid.toString())
        }
    }
    ...
    private fun enableCharacteristicNotification(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic) {
        ...
        if (characteristicWriteSuccess) {
            mClientActionListener.log("Characteristic notification set successfully for " + characteristic.uuid.toString())
            ...
            mClientActionListener.logError("Characteristic notification set failure for " + characteristic.uuid.toString())
        }
    }
